...
open class JZBaseWeekView: UIView {
    ...
    */
    ...
    open func getCurrentEvent(with indexPath: IndexPath) -> JZBaseEvent? {
        ...
        if notAllDayEventsBySection[date] == nil {
            return nil
        }
        
        if allEventsBySection[date] == nil {
            return nil
        }
        
        if indexPath.row >= notAllDayEventsBySection[date]!.count {
            return nil
        }
        
        if indexPath.row >= allEventsBySection[date]!.count {
            return nil
        }
...
extension JZBaseWeekView: UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    open func scrollViewWillEndDragging(_ scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
        ...
        perform(#selector(self.endOfScroll), with: nil, afterDelay: Double(0))
    }
